...
from ..dec07 import run_phases


@pytest.mark.parametrize("program, phases, result", (
...
def test_run_phase(program, phases, result):
    assert run_phases(program, phases) == result
...
def test_run_feedback(program, phases, result):
    assert run_phases(program, phases) == result
