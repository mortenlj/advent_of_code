        ...
        for j in (-1, 0, 1):
            for i in (-1, 0, 1):
                ...
                if nx < 0 or ny < 0 or nnx < 0 or nny < 0:
                    continue
                if nx >= board.size_x or ny >= board.size_y or nnx >= board.size_x or nny >= board.size_y:
                    continue
                other = Node(Vector(nnx, nny))
                if board.get(nx, ny) == "#" and board.get(nnx, nny) in (".", "E") and other in path and pos_to_idx[other] > pos_to_idx[node]:
